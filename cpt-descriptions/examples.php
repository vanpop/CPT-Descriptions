 ...
 */
function my_change_cptd_menu_title( $title, $post_type ) {
	return $post_type->name . ' Listing Page';
}
add_filter( 'cptd_menu_label', 'my_change_cptd_menu_title', 10, 2 );
 ...
 */
function my_change_cptd_settings_page_title( $title, $post_type ) {
    return 'Edit ' . $post_type->label . ' Listing Page';
}
add_filter( 'cptd_admin_title', 'my_change_cptd_settings_page_title', 10, 2 );
 ...
 */
function my_add_cptd_post_types( $post_types ) {
...
add_filter( 'cptd_post_types', 'my_add_cptd_post_types' );

/**
 * modify the admin menu title
 * 
 * @return new array of post types to use
 */
function my_change_cptd_post_types() {
	$args = array(
		'has_archive' => true,
		'_builtin' => false
	);
...
add_filter( 'cptd_post_types', 'my_change_cptd_post_types' );
